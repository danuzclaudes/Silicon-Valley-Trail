...
public class Adventure {
	...
	public Adventure(Difficulty mode, Squad squad) {
		...
		addTravelObserver(squad.getKnapsack()); // knapsack is observer of Travel
	...
	// travel is now threaded: default delay of 100ms per day
	// cause a new thread to start
	// => will inform any registered TravelObserver objects as each day passes
	public void travel(City next_city, int delay) {
		...
		Thread travel_thread = new Thread(new Runnable() {
			public void run() {
				...
				travelling = false;
				 /* when distance <- 0, i.e.
				  * after the destination city is reached, 
				  * there is one more notification made to 
				  * each registered travel observer 
				  * with distance to destination set to 0. 
				  * => does not count as a day of travel 
				  */
				for (TravelObserver o : travel_observers) {
